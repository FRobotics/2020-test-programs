...
//JAS This is a modified version of the release version as of 3/19/2021.
// It contains a modified calibration routine.  This may help eliminate 
// some drift.  The class name is changed so it doesn't conflict with
// the default driver.
...
@SuppressWarnings("unused")
public class ALT_ADIS16448_IMU extends GyroBase implements Gyro  {
  ...
  //JAS added
  private double m_accum_gyro_dt[];
  ...
  private static class AcquireTask implements Runnable {
    private final ALT_ADIS16448_IMU imu;

    public AcquireTask(final ALT_ADIS16448_IMU imu) {
      this.imu = imu;
    }
   ...
   *
   */
  public ALT_ADIS16448_IMU(final IMUAxis yaw_axis, SPI.Port port, int cal_time) {
    m_yaw_axis = yaw_axis;
    m_spi_port = port;
  ...
  private void initOffsetBuffer(int size) {
    ...
    //JAS added
    m_accum_gyro_dt = new double [size];
  ...
  public void calibrate() {
    synchronized (this) {
      ...
      for (int i = 0; i < gyroAverageSize; i++) {
        ...
        //JAS added
        m_gyro_accum_dt += m_accum_gyro_dt[i];
      }
      //JAS replaced
      ...
      //JAS added replacement
      m_gyro_offset_x = m_gyro_accum_x / m_gyro_accum_dt;
      m_gyro_offset_y = m_gyro_accum_y / m_gyro_accum_dt;
      m_gyro_offset_z = m_gyro_accum_z / m_gyro_accum_dt;
      m_integ_gyro_x = 0.0;
      m_integ_gyro_y = 0.0;
      m_integ_gyro_z = 0.0;
          ...
          if (calc_crc == imu_crc) {
            ...
            synchronized (this) {
              ...
              if(m_first_run) {
                ...
                //JAS added
                m_accum_gyro_dt[bufferAvgIndex] = m_dt;
                // Increment counter
                m_accum_count++;
              }
              if (!m_start_up_mode) {
